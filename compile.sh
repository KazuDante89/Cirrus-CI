...
build_kernel()
{
  ...
  make -j$(nproc --all) $MAKE_PARAMS INSTALL_MOD_PATH=modules INSTALL_MOD_STRIP=1 modules_install
	...
	then
	    ...
	    find "$KERNEL_DIR"/out/modules -type f -iname '*.ko' -exec cp {} $AK3_DIR/modules/system/lib/modules/ \;
	fi

		BUILD_END=$(date +"%s")
		DIFF=$((BUILD_END - BUILD_START))

		if [ -f "$KERNEL_DIR"/out/arch/arm64/boot/$kernel ]
		then
			...
			then
				...
				python3 "$KERNEL_DIR/scripts/dtc/libufdt/mkdtboimg.py" \
